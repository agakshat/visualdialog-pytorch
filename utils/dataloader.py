...
class validate(data.Dataset): # torch wrapper
    ...
    def __getitem__(self, index):
        ...
        fact = np.zeros((self.rnd, self.ques_length+self.ans_length+2))

        opt_ans = np.zeros((self.rnd, self.negative_sample, self.ans_length+1))
        ans_ids = np.zeros(self.rnd)
        opt_ans_target = np.zeros((self.rnd, self.negative_sample, self.ans_length+1))
        ...
        for i in range(self.rnd):
            ...
            fact[i, :q_len] = self.ques[index, i, :q_len]
            fact[i, q_len] =  self.vocab_size
            fact[i, q_len+1:q_len+1+a_len] = self.ans[index, i, :a_len]
            fact[i, q_len+1+a_len] = self.vocab_size
        ...
        return img, his, ques, ans, ans_target, quesL, opt_ans, \
                    opt_ans_target, ans_ids, ans_len, opt_ans_len, img_id, ques_inp, ques_target, fact, cap
